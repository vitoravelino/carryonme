...
import static server.JerseyHandlerHelpers.getVirtualHosts;
...
public class HandlersBuilder {
    ...
    private RequestLog createRequestLog() {
        String logFilename = Configuration.getRequestLogFolder() + separatorChar
    ...
    private Collection<Handler> createStaticHandlers() {
        ...
        for (String name : Configuration.getStaticResources()) {
    ...
    private Handler createGuiceServletHandler() {
        ...
        if (virtualHosts.length != 0) {
            servletHandler.setVirtualHosts(virtualHosts);
        }
