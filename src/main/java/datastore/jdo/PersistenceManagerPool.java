    ...
    private static class PMFWrapper {
        ...
        static {
            if (Configuration.isDebugModeEnabled()) {
                pmFactory = JDOHelper.getPersistenceManagerFactory(Configuration.getPMFNameDebug());
            } else {
                pmFactory = JDOHelper.getPersistenceManagerFactory(Configuration.getPMFNameProduction());
            }
        }
